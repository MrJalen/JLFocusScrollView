...
#define scrollView_Width self.scrollView.frame.size.width  //scrollView的宽度

#define scrollView_Height self.scrollView.frame.size.height  //scrollView的高度

#define CONTENTSIZE_COUNT    5  //最大的个数用于计算contentSize

#define CONTENTSIZE_CURRENT_COUNT 2  //计算当前contentOffset，位于中间位置

#define pageControl_bottom_Margin    10  //pageControl底部间距

#define NSTimer_Time 3.0  //默认定时时间

@interface JLFocusScrollView () <UIScrollViewDelegate>
...
@implementation JLFocusScrollView
...
- (void)setupScrollViewContentSize {
    if(self.images.count > 1) {
        switch (self.style) {
            case JLFocusScrollViewScrollStyle_Fade:
            case JLFocusScrollViewScrollStyle_3DRotation:
            case JLFocusScrollViewScrollStyle_pageCurl:
            case JLFocusScrollViewScrollStyle_pageUnderCurl:
            case JLFocusScrollViewScrollStyle_rippleEffect:
            case JLFocusScrollViewScrollStyle_oglFlip:
            {
...
#pragma mark - 指示器方法
- (void)setPageControlPosition:(JLFocusScrollViewPageControllPosition)pageControlPosition {
    _pageControlPosition = pageControlPosition;
    self.pageControl.hidden = (pageControlPosition == JLFocusScrollViewPageControllPosition_Hidden);
    ...
    if(_pageControlPosition == JLFocusScrollViewPageControllPosition_BottomCenter || _pageControlPosition == JLFocusScrollViewPageControllPosition_None) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    }else if (offsetX >scrollView_Width * 2) {
        //右边滑动
        switch (self.style) {
            case JLFocusScrollViewScrollStyle_Fade:
            {
...
- (UIImage *)placeHoderImage {
    if(_placeHoderImage == nil) {
        _placeHoderImage = [UIImage imageNamed:@"fengjing02"];
    }
